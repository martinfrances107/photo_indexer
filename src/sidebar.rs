...
#[component]
pub fn Sidebar(metadata: Signal<Option<Vec<Field>>>) -> impl IntoView {
    view! {
      <Transition
        fallback=move || view! {<p>"SideBar..."</p>}
     >
    {move || {

        metadata.get().map_or_else(|| view! {<div id="side-menu-empty" class="w-0"></div>}, |data| view! {
